#!/usr/bin/env bash
# FINDING SECURITATE (audit runda 5/6): identitate ALLOWLISTATA (kube-system) COMPROMISA evadeaza regulile gated
# (recon/destruct/hijack/persist exonereaza allowlist-ul). Cream SA-uri REALE in kube-system (system:serviceaccount:
# kube-system:compromised-ctrl-N) si actionam CU TOKENUL LOR (NU impersonare -> auditul cheia pe identitatea kube-system).
# Tactici RATE-based PURE (fara CRB/exec/secrete-multi-ns -> NU declanseaza F) ca sa demonstram gaura. bash 3.2.
# SIGURANTA: distructiv DOAR in lab-victim + decoys + cleanup complet (inclusiv SA-urile din kube-system).
set -uo pipefail
...
echo ">> GATA compromised-allowlist. sesiuni totale: $(grep -cE 'SESSION [0-9]+ START' "$SF")"

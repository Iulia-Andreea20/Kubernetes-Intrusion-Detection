#!/usr/bin/env bash
# WILSON LB PUSH: aduce fiecare categorie held-out la N>=10 (=> Wilson95 LB >=72%, peste pragul 70%), cu VARIATIE
# comportamentala genuina (NU detonari byte-identice). bash 3.2. SIGURANTA: distructiv DOAR in lab-victim + decoys.
#   Stratus escaladare x5 -> 13  (module ROTITE: priv-esc + cred-access)         [TOOL-disjunct, dovada tare]
#   Stratus persistence  x9 -> 10 (3 module DISTINCTE x3: crb / client-cert / token) [TOOL-disjunct, tactica noua]
#   Stratus lateralext   x5 -> 6  (steal-token + create-token; 1 tehnica)         [TOOL-disjunct; variet. limitata]
#   lowslow x6 ->10  lateral x5 ->10  impact x4 ->10  evasion x4 ->10  escv x7 ->10 [IDENTITY-disjunct sintetic, variat]
set -uo pipefail
BIN=/tmp/rtbin
OUT="$(cd "$(dirname "$0")" && pwd)/reference_dataset_v2"; SF="$OUT/sessions.txt"; mkdir -p "$OUT"; touch "$SF"
...
for mods in "$em1" "$em2" "$em3" "$em4" "$em5"; do
  sess "STRATUS-esc #$k (tool-disjunct, module variate)"
...
# ============ ESCV (escaladare variata sintetica) x7 -> 10 ============
...
echo ">> GATA Wilson push. sesiuni totale: $(grep -cE 'SESSION [0-9]+ START' "$SF")"

#!/usr/bin/env python3
# export_v2 — set v1.2: event-level din LA -> ferestre de 20 -> trasaturi A (15 vechi + invariante la densitate).
# Eticheteaza fiecare fereastra cu UNEALTA (synthetic/stratus/rakkess/peirates) pt split tool-disjunct.
# Citeste sessions.txt (granite per sesiune) si interogheaza LA chunked (az rest). Scrie ref_v2_all.csv.
import subprocess, json, csv, sys, os
from collections import defaultdict, deque
HERE = os.path.dirname(os.path.abspath(__file__))   # portabil (nu mai e cale absolută hardcodată)
OUT = HERE + "/reference_dataset_v2"
# CID = customerId-ul workspace-ului Log Analytics. La RECREARE (workspace nou) ai alt CID:
# setează LA_WORKSPACE_ID (scris de setup_aks.sh în env.generated) SAU editează default-ul. Vezi RESTORE.md.
CID = os.environ.get("LA_WORKSPACE_ID", "39628155-ae16-4624-90d3-41d58489f713")  # default = workspace law-ids-aks curent
SELF_REVIEW = {"selfsubjectaccessreviews","selfsubjectrulesreviews"}
RBAC = {"clusterroles","clusterrolebindings","roles","rolebindings"}
SEQ = 20
# atac = identitatile adverse (synthetic) + uneltele reale de atac
...
def tool_of(u):
    ...
    if "adversary-stealth" in u: return "lowslow"   # escaladare DILUATA (held-out, attacker-disjunct)
    if "adversary-imp" in u: return "impact"        # IMPACT (adversary-impact, adversary-imp-1/2/3)
    if "adversary-eva" in u: return "evasion"       # DEFENSE EVASION (adversary-evasion, adversary-eva-1/2/3)
    if "adversary-lat" in u: return "lateral"       # LATERAL/impersonare (adversary-lateral(2), adversary-lat-1/2/3)
    if "redteam-persist" in u: return "persistence" # PERSISTENCE via Stratus (extern, held-out, tool-disjunct)
    if "redteam-lat-ext" in u: return "lateralext"  # LATERAL via token-reuse (Stratus extern, tool-disjunct)
    if "adversary-escv" in u: return "escv"         # ESCALADARE regenerata cu variatie (sintetic)
    if "compromised-ctrl" in u: return "compromised" # identitate ALLOWLISTATA (kube-system) COMPROMISA — testeaza gaura granitei de incredere
    return "synthetic"
def is_attack(u):
    return tool_of(u) in ("stratus","rakkess","peirates","lowslow","impact","evasion","lateral","persistence","lateralext","escv","compromised") or any(k in u for k in ATTACK_SYN)
...
def q_window(start, end):
    ...
    print("  !! q_window esuat:", (r.stderr or "")[-150:]); return []

# citeste sessions.txt: "SESSION i START ts" / "SESSION i END ts"
spans = []
...
print(f">> {len(spans)} sesiuni din sessions.txt")
...
rows=[]
for i,s,e in spans:
    rr=q_window(s,e); rows.extend(rr); print(f"  sesiune {i}: {len(rr)} ev")
print(f">> total {len(rows)} evenimente brute")
...
for ts,log in rows:
    ...
    real=(ev.get("user") or {}).get("username","")                  # identitatea REALA (cine s-a autentificat)
    imp=((ev.get("impersonatedUser") or {}).get("username") or "")  # identitatea impersonata (efectiva)
    is_imp=1 if (imp and imp!=real) else 0
    a=real or imp                                                   # cheia ferestrei = atacatorul REAL (nu victima impersonata)
...
def is_csr(x): return x["resource"]=="certificatesigningrequests"          # persistence via client-cert (CSR self-approve)
def is_tokenreq(x): return x["verb"]=="create" and x["resource"]=="serviceaccounts" and x["sub"]=="token"  # TokenRequest abuse
...
def featurize(evs):
    ...
    for x in evs:
        ...
        himp=int(any(y["is_imp"] for y in h)); nimp=len({y["imp"] for y in h if y["is_imp"]})  # impersonare (lateral movement)
        ncw=sum(1 for y in h if wl_create(y))  # creari de workload (impact/hijack — pt regula de suport)
        hcsr=int(any(is_csr(y) for y in h)); htok=int(any(is_tokenreq(y) for y in h))  # persistence/cred (regula de suport)
...
print(f">> scris {OUT}/ref_v2_all.csv: {n_rows} ferestre | pe unealta: {dict(by_tool)}")
print(f">> features: {len(OLD)} vechi + {len(NEW)} noi (A) = {len(OLD)+len(NEW)}")

...
are reported too, as honest negatives: a tool exists for both and the classifier misses both.
    ...
    source detection/bin/activate && python src/model/eval/eval_model_only_standalone.py
"""
import csv, os
...
DS     = os.path.join(REPO, "src/dataset/reference/ref_v2_all.csv")
FIGDIR = os.path.join(REPO, "docs/results"); os.makedirs(FIGDIR, exist_ok=True)
